...
const startServer = function(params) {
  ...
  const admin = function(req, res, next) {
    if (app.securityhandler.isAdmin(req)) {
      ...
      let a, u;
      if (!argv.admin) { a = "no admin specified"; }
      if (!req.session?.passport?.user && !req.session?.email && !req.session?.friend) { u = "not logged in"; }
  ...
  const owner = function(req, res, next) {
    if (!app.securityhandler.isAuthorized(req)) { return res.status(401).send("must be owner"); }
    return next();
  };
  ...
  app.post('/plugin/register/new', owner, farm, function(req, res) {
    ...
    if (!(data = req.body.data)) { return e400("Missing data"); }
  ...
  app.get('/plugin/register/using', farm, owner, function(req, res) {
    ...
    return fs.readFile(`${argv.status}/owner.json`, 'utf8', function(err, owner) {
      ...
      return fs.readdir(where, {withFileTypes:true}, function(err, files) {
        const have = files.filter(file => file.isDirectory() && file.name.match(new RegExp(`^[a-z][a-z0-9]{1,7}\\.${like}$`)));
  ...
  app.post('/plugin/register/delegate', owner, farm, function(req, res) {
    ...
    if (!(data = req.body.data)) { return e400("Missing data"); }
    if (!(context =req.body.context)) { return e400("Missing context"); }
    if (!data.owner) { return e400("Missing owner name"); }
  ...
  app.post('/plugin/register/custom', owner, farm, function(req, res) {
    ...
    if (!custom) { return e501("No register module"); }
    return custom.post(argv, data, function(err, status) {
      if (err) { return e400(status); }
